...
<body>
			...
			<div class="panel panel-default" style="width:1540px; height:1000px;">
				<div  style="padding-top:50px; padding-left:70px; padding-right:70px;">
					...
					<div style="margin-top:100px;">
						<table class="table">
							<tr>
								<th>번호</th><th>기관등록번호</th><th>담당자</th><th>연락처</th><th>이메일</th><th>재무제표</th><th>신청일자</th><th>상태</th>
							</tr>
							<c:forEach items="${list}" var="c" varStatus="l">
								<tr>
									<td>${(reqPage-1)*10 + l.count }</td>
									<td><a style="cursor:pointer;" data-toggle="modal" data-target="#dataModal" data-companynumber="${c.companyNumber}"> ${c.companyNumber }</a></td>
									<td>${c.memberId }</td>
									<td>${c.memberPhone }</td>
									<td>${c.memberEmail }</td>
									<td>파일</td>
									...
									<td>
										<button class="btn-sm btn-primary" onclick="enrollCompany('${c.memberId}', 0, this);">승인</button>
										<button class="btn-sm btn-primary" onclick="enrollCompany('${c.memberId}', 1, this);">거부</button>
									</td>
								</tr>
							</c:forEach>
						</table>
					...
					<div class="modal-body" style="height:400px;">
						...
						<div id="content" style="height:300px; width:430px; margin:0 auto; margin-top:30px; display:none;">
							<table class="table" border=1>
								<tr>
									<td class="title">단체명</td><td id="companyName"></td>
								</tr>
								<tr>
	...
	<script>
		...
		$('#dataModal').on('show.bs.modal', function(event) {			
	        companyNumber = $(event.relatedTarget).data('companynumber');
		...
		//후원단체 승인 및 거부
		function enrollCompany(memberId, type, obj){
			var bool = 0;
